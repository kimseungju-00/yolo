...
# YOLOv8 모델 로드
model = YOLO('yolov8x.pt')  # 최신 YOLOv8 모델 로드
model.overrides['conf'] = 0.25  # 신뢰도 임계값 조정
...
while True:
    ...
    # YOLO로 공 객체 탐지
    results = model(frame)  # YOLOv8 모델 사용
    ...
    for obj in objects:
        ...
        # 공(클래스 ID 32)일 때만 처리 (YOLOv8 모델에서 클래스 ID 확인 필요)
        if label == 'sports ball':  # 클래스 이름이 'sports ball'일 때
            ...
            # 작은 객체 필터링 (필터링 제거)
            # if ball_width < 20 or ball_height < 20:
            #    continue
            ...
            ball_trajectories.append((x1 + ball_width // 2, y1 + ball_height // 2, current_time))

            # 최근 두 지점 간 속도 계산
            if len(ball_trajectories) > 1:
                prev_x, prev_y, prev_time = ball_trajectories[-2]
                time_diff = current_time - prev_time
                if time_diff > 0:
                    distance_diff = calculate_distance(ball_width, focalLength)
                    speed = calculate_speed(distance_diff, time_diff)
                    print(f"Speed: {speed:.2f} km/h")

            # 프레임에 궤적 시각화
            for i in range(len(ball_trajectories) - 1):
                pt1 = (ball_trajectories[i][0], ball_trajectories[i][1])
                pt2 = (ball_trajectories[i + 1][0], ball_trajectories[i + 1][1])
